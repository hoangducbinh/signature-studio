...
const INTERNAL_SCALE = 4; // xử lý hi‑res để mượt mép

let origImg = null;
let mask = null; // Uint8ClampedArray alpha 0..255 ở work space
let colorMap = null; // Uint8ClampedArray RGBA cho từng pixel
let originalColorMap = null; // Backup của colorMap ban đầu
let workW = 0, workH = 0;
const workCanvas = document.createElement('canvas');

// Trạng thái chọn vùng
let isSelectMode = false;
let selectionMode = 'rect'; // 'rect' hoặc 'brush'
let selectedRegion = null; // {x, y, w, h} cho rect hoặc points[] cho brush
let isDrawing = false;
...
function makeSoftMask(gray, threshold, feather = 24) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (g <= tLo) a = 255;
      ...
      // Sử dụng smoothstep để làm mượt hơn
...
function renderPreview(){
  ...
  if(colorMap) {
    // Sử dụng colorMap nếu có (cho màu theo vùng cụ thể)
    for(let i=0, j=0; i<img.data.length; i+=4, j++){
      const a = mask[j];
      if(a > 0) {
        img.data[i] = colorMap[i];     // Đỏ
        img.data[i+1] = colorMap[i+1]; // Xanh lá
        img.data[i+2] = colorMap[i+2]; // Xanh dương
  ...
  // Thêm một lần blur nhẹ cuối cùng để làm mượt hơn
  ...
  // Vẽ overlay chọn vùng nếu đang ở chế độ chọn
...
function resetUI(){
  ...
  selectModeBtn.textContent = '🖱️ Chọn vùng';
...
function applyRegionChanges() {
  ...
  // Chuyển đổi tọa độ chọn vùng sang không gian làm việc
    ...
    for(let y = workRegion.y; y < workRegion.y + workRegion.h; y++) {
      for(let x = workRegion.x; x < workRegion.x + workRegion.w; x++) {
        if(x >= 0 && x < workW && y >= 0 && y < workH) {
          ...
          if(mask[idx] > 0) {
            // Áp dụng thay đổi màu
            colorMap[colorIdx] = r;
            colorMap[colorIdx + 1] = g;
            colorMap[colorIdx + 2] = b;
            
            // Áp dụng thay đổi độ dày nét
            if(strokeChange !== 0) {
...
function resetRegion() {
  ...
  if(selectionMode === 'rect') {
    // Reset vùng hình chữ nhật về màu gốc
    for(let y = workRegion.y; y < workRegion.y + workRegion.h; y++) {
      for(let x = workRegion.x; x < workRegion.x + workRegion.w; x++) {
        if(x >= 0 && x < workW && y >= 0 && y < workH) {
          const idx = y * workW + x;
      ...
      // Reset chọn vùng bằng cọ về màu gốc
    const brushRadius = selectedRegion.brushSize * scale / 2;
    for(let i = 0; i < selectedRegion.points.length; i++) {
      const point = selectedRegion.points[i];
      const workX = Math.round(point.x * scale);
...
strokeNum.addEventListener('change', ()=>{
  ...
  if(!isNaN(val) && val >= -5 && val <= 5){
    ...
    // Reset về giá trị của slider nếu nhập sai
    strokeNum.value = stroke.value;
  }
});
...
color.addEventListener('change', ()=>{
  colorHex.value = color.value;
          // Cập nhật colorMap với màu toàn cục mới
  if(colorMap && mask) {
    const {r,g,b} = hexToRgb(color.value);
    for(let i=0, j=0; i<colorMap.length; i+=4, j++){
      if(mask[j] > 0) {
...
colorHex.addEventListener('keydown', (e)=>{
  if(e.key === 'Enter'){
    ...
    if(/^#[0-9A-Fa-f]{6}$/.test(hex)){
      ...
      // Nếu không phải hex hợp lệ, reset về giá trị của color picker
      colorHex.value = color.value;
    }
  }
});
